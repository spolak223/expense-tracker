    ...
    def sort_expenses(self, selected):
        ...
        if selected == "Date Asc":
            for key in range(len(self.expenses)):
                ...
                date = date.split("/")
                self.expenses = sorted(self.expenses, key=lambda x: x["date"], reverse=False)
            return self.expenses
        elif selected == "Date Asc":
            for key in range(len(self.expenses)):
                date = self.expenses[key]['date']
                date = date.split("/")
                self.expenses = sorted(self.expenses, key=lambda x: x["date"], reverse=True)
            return self.expenses
        else:
